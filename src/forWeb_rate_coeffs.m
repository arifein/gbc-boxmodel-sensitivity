function k_mat = forWeb_rate_coeffs(k_factors, Lriver_FHgP, IHgD_pristine, IHgP_pristine)
    %==========================================================================
    % OBJECTIVE
    ...
    Dep_oHgII  = 3986;      % Hg(II) deposition to the ocean (Feinberg et al. 2022)
    Dep_tHgII  = 790;            % Hg(II) deposition to land (Feinberg et al. 2022)
    Dep_tHg0   = 2170;            % Hg(0) deposition to land  (Feinberg et al. 2022)

    % Hg0 air - sea exchange
    Ev_Hg0_ocs = 5165; % gross ocean evasion to the atmosphere (Feinberg et al. 2022)
    Upt_oHg0   = 1961; % gross uptake of Hg0 from the atmopshere (Feinberg et al. 2022)
    netEv_Hg0  = Ev_Hg0_ocs - Upt_oHg0; % net evasion from surface ocean to atmosphere (Feinberg et al. 2022)
    ...
    fdep_ta    = 1 - fdep_tf - fdep_ts;             % the fast armored pool
    ...
    % save out factors into a single matrix
    k_mat_prescale = [k_A_oHgII, k_A_tHgII, k_A_tHg0, k_A_oHg0, ...
        fdep_tf, fdep_ts, fdep_ta, k_Oc_ev, k_Oc_sp1, k_Oc_vsi, ...
        k_Oc_sp2, k_Oc_vis, k_Oc_vid, k_Oc_sp3, k_Oc_vdi, k_Te_rf, k_Te_p, ...
        k_T_exfs, k_T_exfa, k_Te_BBf_1, k_Te_BBf_2, k_Te_rs, k_T_exsf, k_T_exsa, ...
        k_Te_BBs_1, k_Te_BBs_2, k_Te_ra, k_T_exaf, k_T_exam, k_Te_BBa_1, k_Te_BBa_2, ...
